...
# ╔═╡ 40019946-bf5a-4731-ab1c-36614343cfd1
begin
	# package imports 
	using GLMakie
	using Plots
...
# ╔═╡ 72760a91-a9c1-4dea-ad4d-b2f318a7759a
md"""
# Problem Set 0 
"""

# ╔═╡ 0a051eb6-1cd4-4ade-bc51-735d83bd25c3
md"""
## Exercise 1 
"""

# ╔═╡ de1556b5-b548-463f-97ba-e9603099c179
md"""
#### Importing the relevant packages
"""

# ╔═╡ 8d4fe6cc-b867-49f2-93d2-14aff2a985ec
md"""
### Utility functions
"""

# ╔═╡ 31c2ff48-cafb-46f7-85a9-a88ebe2cc794
md"""
#### Setting up parameters
We assume that $\mu$ is the substitution parameter in the CES model. It is thus defined as follows (with $\sigma$ the elasticity of substitution) : $$\mu = \frac{\sigma-1}{\sigma}$$
"""

# ╔═╡ 1a422b69-56a6-4bad-804d-78fc5fe5dbf1
begin
	beta = 0.4 
	sigma = 2.0 
	psi = 1.0
...
# ╔═╡ 1d5b9b15-4fd6-460e-a14c-003cc67af9d4
begin
	sld = @bind alpha PlutoUI.Slider(0.0:0.01:1.0; default = 0.3, show_value = true)
	lbl = HTML("<div style='font-weight:600; margin-bottom:6px;'>parameter: α</div>")
	(lbl, sld)
end 

# ╔═╡ 355ce89b-798f-4294-807c-9957faebb727
md"""
#### 1. Write the utility functions, with c and l as only inputs.
"""

# ╔═╡ e86bbce2-f19d-4142-bd65-26b967a4b618
begin
...
# ╔═╡ 624ec779-b8f3-474a-b165-67924139d947
md"""
#### 2 & 3. Define c as a function of l through the budget constraint 
"""

# ╔═╡ 237b2fc8-d4d9-4719-a525-98bb611d6d5b
begin
	y = 0.1
	w = 1.0
	...
	# computes the utility value for all four utility functions based on the above defined parameter range 
...
# ╔═╡ 039d247c-1e25-4d7c-97a2-adfdcb061075
md"""
#### 4. Try it ! 
"""

# ╔═╡ 3789d5e6-5590-4f11-9c21-d6eb4f515896
begin
	budget = budgeting(1,3,0.2)
end

# ╔═╡ 4afdc14b-30c0-49a3-8fc6-c4885f65f34e
md"""
### Brute-force search for optimal labor
"""

# ╔═╡ 8f166066-3ea3-4493-92c3-42f368ceb92d
begin
	nl = 101
...
# ╔═╡ 041b4e0c-54e4-4ac7-84f6-905d027a2cd7
md"""
#### 5. Compute the utility for each function and for all choices 
"""

# ╔═╡ e056ab18-06ef-42d8-9e92-2c6637fc4459
begin
...
# ╔═╡ f3e0e88c-3351-4177-be51-7caa79ccb9ef
md"""
#### 6. Plot it 
"""

# ╔═╡ 498aa481-6ce0-4868-b29e-8a240e3dd555
begin
	P1 = Plots.plot(utilities.l,[utilities.CD,utilities.CES,utilities.FRISCH,utilities.SG], labels = ["CD" "CES" "FRISCH" "SG"], title = "Utilities as a function of labor supply",titlefont = font(8), tickfontsize = 7, legendfontsize = 7)
	Plots.savefig(join([filepath,"/q6_utility_comparison.pdf"],""))
    P1
end

# ╔═╡ 1faeae62-ff9f-4aae-8716-7203fd246094
md"""
#### 7. Find the maximum
"""

# ╔═╡ b7702df7-e87e-40d7-aacf-6f0fa91b4041
begin
...
# ╔═╡ 495fd71c-6e80-4294-99df-8a774f5023fd
md"""
### Comparative statics over income and wages
"""

# ╔═╡ bfd200d8-e608-47ee-a844-49ce8e9b82be
md"""
#### 9. create a grid for wealth and income
"""

# ╔═╡ 8e231750-0497-4ff9-9ca1-6f356d0c419b
begin
	ny = 6
	nw = 11
	ygrid = LinRange(0.0,0.5,ny);
	wgrid = LinRange(1.0,2.0,nw);
	V_opt_init = zeros(nw,ny);
	l_opt_init = zeros(nw,ny);
	i_opt_init = zeros(Int64,nw,ny);
end

# ╔═╡ 66785446-39f4-43de-b4ab-a657e14360cb
md"""
#### 10. Compute the optimal labor supply for each (w,y)
"""

# ╔═╡ 3c4e9809-b86f-4613-b4bd-eca9a27af675
begin
	...
	for iy in 1:ny
	    for iw in 1:nw
	        w = wgrid[iw]
	        y = ygrid[iy]
	        l_opt_init[iw,iy], V_opt_init[iw,iy] =  max_util(w,y,lgrid,nl)
	        push!(df_optimal_supply, (w,y,V_opt_init[iw,iy],l_opt_init[iw,iy]))
	    end
	end
	df_optimal_supply
end

# ╔═╡ a6dcc51b-22fd-4b12-a33b-40c311e0b51d
md"""
### Plots for comparative statics
"""

# ╔═╡ ed201efa-c4cd-4127-ba3e-0f4af9ca2ba8
md"""
#### 11. plot optimal labor supply vs. wealth for fixed wages
"""

# ╔═╡ 72f7de20-093a-4109-bacb-1ae9e1e4ee64
begin	
	V_opt = zeros(nw,ny,4);
	l_opt = zeros(nw,ny,4);
	...
	for i in 1:length(w1)
	    ...
	    P = Plots.plot(df_plot.y, [df_plot.opt_l_CD,df_plot.opt_l_CES,df_plot.opt_l_FE,df_plot.opt_l_SG],titlefont = font(5), labels = ["CD" "CES" "FRISCH" "SG"],title = (["Optimal labor supply as a function of wealth for incomjoine w1 = ",round(wgrid[i]; digits = 2)]," "), tickfontsize = 5, legendfontsize = 4)
		plots_w = push!(plots_w,P)
	end
	Plots.plot(plots_w..., layout = length(w1))
	Plots.savefig(join([filepath,"q11_plot_w_",alpha,".pdf"],"") )
	Plots.plot!()
end 

# ╔═╡ 30c7c8c9-2ba5-4bc1-8530-608e596b450a
md"""
#### 12. Compare the optimal choices for each utility function!
"""

# ╔═╡ 248e7b25-5129-4ec3-b6eb-4a62b795d3a6
begin
	...
	Plots.savefig(join([filepath,"q12_plot_y_",alpha,".pdf"],""))
	Plots.plot!()
end

# ╔═╡ 4c718c12-db5d-443f-9351-15927328c6af
md"""
...
manifest_format = "2.0"
project_hash = "3fe979e13d0346019d35872a8d6048160a916f1e"
